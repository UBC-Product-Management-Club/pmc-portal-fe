...
enum Pages {
    USER_INFO, PAYMENT, PAYMENT_SUCCESS
}
...
export default function Onboarding() {
    ...
    useEffect(() => {
        paymentService.getMembershipFee()
        .then(setFee)
    ...
    const charge = useMemo(() => {
      if (user && fee) {
        return formatPrice(user.university === Universities[0] ? fee.ubcPrice : fee.nonUbcPrice)
      }
    }, [user, fee])
            ...
            <Container data-testid="onboarding">
                <Content>
                        ...
                        <>
                            ...
                            <UserDataForm 
                                ...
                                onSubmit={(data: UserDataFromUser) => {
                                    try {
                                        update({type: ActionTypes.UPDATE, payload: UserDataFromUserSchema.parse(data)})
                                        ...
                                        if (error instanceof ZodError) {
                                            ...
                                            console.error("An error occurred. Try inputting information again")
                                        }
                                    }
                                }} 
                                hasWaiver
                    ...
                    {currPage === Pages.PAYMENT && fee && user && (
                        <>
                            <PaymentHeader>To become a PMC member for the 2025/2026 academic year, a {charge} membership fee is required.</PaymentHeader>
                            <Payment
                              data-testid="payment"
                              ...
                              options={{type: PaymentType.MEMBERSHIP, university: user.university!}}
                              />
                        </>
                    )}
                    {currPage === Pages.PAYMENT_SUCCESS && (
                      <>
                          <ContentHeader>Welcome to PMC {user?.firstName}! <span style={{fontSize: 'x-large'}}>🥳</span></ContentHeader>
                          <PaymentSuccess>
                                <img src={checkmark} width={40}/>
                                <h3>Payment Successful</h3>
                                <p>We've processed your {charge} charge</p>
                                <br/>
                                <Link to="/dashboard" color="white" style={{ "textDecoration": "None"}}><Button>Continue to dashboard</Button></Link>
                        </PaymentSuccess>
                      </>
                    )}
                </Content>
