...
describe('RestClient', () => {
  ...
  it('should perform a POST request with body', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(`${baseUrl}/submit`, expect.objectContaining({
      method: 'POST',
      body: payload,
      credentials: 'include',
  ...
  it('should handle non-JSON responses gracefully', async () => {
    ...
    const result = await client.get<any>('/plain')
  ...
  it('should send PUT request with JSON body', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(`${baseUrl}/update`, expect.objectContaining({
      method: 'PUT',
